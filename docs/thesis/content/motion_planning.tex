...
\begin{figure}[!ht]
    \begin{tikzpicture}[
        ...
        \node (trajectory)      [inout_node, xshift=3.5cm]                              {Input trajectory};
        \node (control)         [inout_node, xshift=7.0cm]                              {Control};
        \node (dynamic_objects) [inout_node, xshift=10.5cm]                             {Dynamic objects};
        % Update nodes
        \node (find_dest)       [update_node, below of=trajectory, xshift=1.75cm]       {Find destination};
		...
		% Static nodes
        \node (static_filter)   [static_node, below of=static_map, yshift=-3cm]         {Filter};
        \node (static_calc_col) [static_node, below of=static_filter]                   {Calculate collision times};
		% Dynamic nodes
        \node (dynamic_filter)  [dynamic_node, below of=dynamic_objects, yshift=-1.5cm] {Filter};
...
This publication happens on topic \textit{/vrcar/manual\_control}, using Ackermann driving messages, which are standard mesages in ROS (\href{http://docs.ros.org/jade/api/ackermann_msgs/html/msg/AckermannDrive.html}{ackermann\_msgs/AckermannDrive} message type defined by the \href{https://wiki.ros.org/ackermann_msgs}{ackermann\_msgs} package).

The ackermann\_msgs/AckermannDrive message format is the following:
...
Among other fields (which are currently not used by the project) it has a \textit{speed} and a \textit{steering\_angle} field - given in m/s and radians. The actuator nodes will use these two fileds to propagate the requested values to the DC motor and the steering servo, and thus, the control pipe has reached its end.